...
export default defineComponent({
  setup() {
    ...
    const addTodo = async()=> {
    await store.dispatch('todolist/store' , {text:state.todoText}).then(() => {
        state.todoText = "";
        getTodos();
      },
      error=>{
    ...
    function markAsDone(todo) {
      axios.post('http://127.0.0.1:8000/api' + '/todo/mark-as-done', {
        id: todo.id
      }).then(() => getTodos());
    }
    function markAsUndone(todo) {
      axios.post('http://127.0.0.1:8000/api'  + '/todo/mark-as-undone', {
        id: todo.id
      }).then(() => getTodos());
    }
    function removeTodo(todo) {
      ...
      axios.post('http://127.0.0.1:8000/api'  + '/delete-todo', {
        id: todo.id
      }).then(() => getTodos());
    }

  const getTodos = async()=> {
    await store.dispatch('todolist/getuser').then(response => {
